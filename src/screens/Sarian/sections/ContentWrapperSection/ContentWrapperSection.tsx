    ...
    <section id="services" className="relative w-full py-12 md:py-16 lg:py-20 px-4 sm:px-6 md:px-8">
      <div className="container mx-auto max-w-7xl">
        <div className="flex items-center justify-center mb-12 md:mb-16 translate-y-[-1rem] animate-fade-in opacity-0">
          <h2 className={`font-bold text-black text-3xl sm:text-4xl md:text-5xl text-center [font-family:'Inter',Helvetica] tracking-[0] leading-normal ${language === 'ar' ? '[direction:rtl]' : '[direction:ltr]'}`}>
            {t('services.title')}
          </h2>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
          {servicesData.map((service, index) => {
            ...
            return (
              <Card
                key={index}
                className={`relative min-h-[500px] sm:min-h-[550px] md:min-h-[600px] lg:min-h-[650px] shadow-[4px_4px_13.8px_#00000040] bg-gradient-to-br ${service.bgGradient} border-0 translate-y-[-1rem] animate-fade-in opacity-0 transition-all duration-300 hover:scale-105 hover:shadow-2xl`}
                style={
                  {
              ...
              >
                <CardContent className="relative h-full p-4 sm:p-6 flex flex-col">
                  ...
                  <div className="flex flex-col gap-4 pb-4">
                    <h3 className={`font-semibold text-black text-2xl sm:text-3xl md:text-4xl text-center [font-family:'Inter',Helvetica] tracking-[0] leading-normal ${language === 'ar' ? '[direction:rtl]' : '[direction:ltr]'}`}>
                      {service.title}
                    </h3>

                    <p className={`font-light text-black text-base sm:text-lg md:text-xl lg:text-2xl text-center [font-family:'Inter',Helvetica] tracking-[0] leading-normal px-2 ${language === 'ar' ? '[direction:rtl]' : '[direction:ltr]'}`}>
                      {service.description}
                    </p>
